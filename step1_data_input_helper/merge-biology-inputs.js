/**
 * 合并 Biology-Gemini提取原始内容.json 和 Gemini-AP Biology step 1.json
 * 创建一个包含所有字段的完整输入文件
 */
...
// 读取两个文件
const originalPath = path.join(__dirname, 'input', 'Biology-Gemini提取原始内容.json');
const geminiPath = path.join(__dirname, 'input', 'Gemini-AP Biology step 1.json');
...
// 输出合并后的文件
const outputPath = path.join(__dirname, 'Biology 完整输入.json');
fs.writeFileSync(outputPath, JSON.stringify(mergedData, null, 2), 'utf8');

console.log('✅ Biology 合并完成！');
